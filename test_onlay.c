...
/* Padding, from randrew/layout#23.
 *
 * A margin is space outside an item and a padding is space inside it, and the
 * two are told apart by who moves: a margin moves the item, a padding moves
 * the item's children and leaves the item where it was. Both halves are
 * checked here, because a padding that only insets the children would still
 * pass a test that looked at the children alone - the container has to report
 * itself larger too, or a parent sizing to its contents will crop it. */
static void
test_padding(void)
{
    lay_context ctx;
    ...
    puts("a padding is space inside an item, not outside it");
    lay_init_context(&ctx);
    lay_reserve_items_capacity(&ctx, 16);
    ...
    box = lay_item(&ctx);
    lay_set_contain(&ctx, box, LAY_ROW);
    lay_set_behave(&ctx, box, LAY_LEFT | LAY_TOP);
    lay_set_paddings_ltrb(&ctx, box, 10, 6, 10, 6);
    lay_insert(&ctx, root, box);

    a = lay_item(&ctx);
    lay_set_size_xy(&ctx, a, 30, 20);
    lay_insert(&ctx, box, a);

    b = lay_item(&ctx);
    lay_set_size_xy(&ctx, b, 40, 20);
    lay_insert(&ctx, box, b);

    lay_run_context(&ctx);

    rb = lay_get_rect(&ctx, box);
    ra = lay_get_rect(&ctx, a);

    check("the box starts where its parent put it", (int)rb.v[0], 0);
    check("the first child starts a left padding in", (int)ra.v[0], 10);
    check("and a top padding down", (int)ra.v[1], 6);
    /* 30 + 40 of children, and 10 either side. */
    check("the box is as wide as its children plus both paddings",
          (int)rb.v[2], 90);
    /* The taller child is 20, and 6 above and below. */
    check("and as tall as the tallest plus both", (int)rb.v[3], 32);

    /* What it is not: a margin. Same numbers on the other field move the box
     * itself and leave its inside alone. */
    lay_set_paddings_ltrb(&ctx, box, 0, 0, 0, 0);
    lay_set_margins_ltrb(&ctx, box, 10, 6, 10, 6);
    lay_run_context(&ctx);
    rr = lay_get_rect(&ctx, box);
    ra = lay_get_rect(&ctx, a);
    check("a margin of the same size moves the box instead",
          (int)rr.v[0], 10);
    check("and its first child sits flush inside it",
          (int)ra.v[0], (int)rr.v[0]);
    check("the box is only as wide as its children", (int)rr.v[2], 70);

    /* And that the getter answers what the setter was given. */
    {
        lay_scalar l = 0, t = 0, r = 0, b2 = 0;
        lay_set_paddings_ltrb(&ctx, box, 1, 2, 3, 4);
        lay_get_paddings_ltrb(&ctx, box, &l, &t, &r, &b2);
        check("what was set comes back: left", (int)l, 1);
        check("top", (int)t, 2);
        check("right", (int)r, 3);
        check("bottom", (int)b2, 4);
    }
